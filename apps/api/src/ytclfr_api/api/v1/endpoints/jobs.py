...
@router.post("/jobs", response_model=SubmitJobResponse, status_code=status.HTTP_202_ACCEPTED)
def submit_job(
    ...
    try:
        ...
        logger.info("Queued job from /jobs endpoint.", extra={"job_id": str(job_id)})
...
def _load_job_status(job_id: UUID, repository: JobRepository) -> JobStatusResponse:
    ...
    try:
        ...
        logger.info("Job status fetched.", extra={"job_id": str(job_id), "status": job.status.value})

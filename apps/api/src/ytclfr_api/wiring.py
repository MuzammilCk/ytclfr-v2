...
- Exposes session_factory and settings as public attributes so
  the enhanced health endpoint can use them.
- Wires FetchVideoResultUseCase to also expose action_output through
  the updated KnowledgeRepository.
- No longer calls Base.metadata.create_all(); schema is managed
  exclusively by Alembic. Run 'alembic upgrade head' before startup.
"""
...
from ytclfr_infra.db import models  # noqa: F401  — ensures ORM models are registered
    ...
    def enqueue_pipeline(self, job_id: UUID) -> None:
        ...
        try:
            self._celery_app.send_task(
                "ytclfr.pipeline.run", kwargs={"job_id": str(job_id)}
            )
...
class AppContainer:
    ...
    Public attributes intentionally exposed for health checks and tests:
    - settings       : validated Settings instance
    - session_factory: SQLAlchemy session factory (used by health check)
    """
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # NOTE: Base.metadata.create_all() is intentionally removed.
        # Schema management is handled exclusively by Alembic migrations.
        # Run 'alembic upgrade head' before starting the API service.
        self.session_factory = build_session_factory(engine)

        self.job_repository: JobRepository = SQLAlchemyJobRepository(
            self.session_factory
        )
        self.knowledge_repository: KnowledgeRepository = SQLAlchemyKnowledgeRepository(
            self.session_factory
        )
        self.video_repository: VideoRepository = SQLAlchemyVideoRepository(
            self.session_factory
        )

        self._celery_app = build_celery_app(settings, app_name="ytclfr-api")
        orchestrator = PipelineOrchestrator(
            CeleryPipelineDispatcher(self._celery_app)
        )
        ...
        self.fetch_video_result_use_case = FetchVideoResultUseCase(

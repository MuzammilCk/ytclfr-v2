...
Uses JobLifecycleService for all state transitions so the task itself
...
from ytclfr_worker.tasks.task_support import get_job_lifecycle_service
...
def run_pipeline(self, job_id: str) -> dict[str, Any]:
    ...
    try:
        lifecycle.mark_running(payload.job_id)
        ...
        logger.exception(
            "Pipeline orchestration failed.", extra={"job_id": str(payload.job_id)}
        )
        try:
            lifecycle.mark_failed(
                payload.job_id,
                error_message=f"pipeline orchestration failed: {exc}",
            )
        except Exception:
            logger.exception("Failed to mark job failed after orchestration error.")
        raise
